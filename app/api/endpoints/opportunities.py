...
async def get_opportunities(
    skip: int = 0, limit: int = 20, sector: Optional[str] = None, db: AsyncSession = Depends(get_db)
):
    """
    ...
    return {"total": total_count, "opportunities": opportunities}
...
async def get_opportunity(opportunity_id: int, db: AsyncSession = Depends(get_db)):
    ...
    query = (
        ...
        .options(selectinload(OpportunityModel.signals))
